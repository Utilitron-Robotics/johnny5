...
class AlohaMiniClient(Robot):
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.is_recording:
            self._capture_frame(action_sent)
            
        return action_sent

    def start_recording(self, path: str):
        """Start recording episodes to the specified path."""
        self.recording_path = path
        self.recorded_frames = []
        self.is_recording = True
        logging.info(f"Started recording to {path}")

    def stop_recording(self):
        """Stop recording and save data."""
        self.is_recording = False
        if self.recorded_frames and self.recording_path:
            # Save as a simple pickle or processed format
            # In a real LeRobot integration, this would use the Dataset writer
            try:
                import pickle
                ...
                logging.info(f"Saved {len(self.recorded_frames)} frames to {self.recording_path}")
            except Exception as e:
                logging.error(f"Failed to save recording: {e}")
        self.recorded_frames = []

    def _capture_frame(self, action: dict):
        """Capture current state and action for recording."""
        if not self.is_recording:
            return
        
        # Combine last observation with the action that was just sent
        frame_data = {
            "timestamp": time.time(),
            "observation": self.last_remote_state,
            "action": action
        }
        # Add images if available (Note: storing raw images in memory list can be heavy)
        frame_data["images"] = self.last_frames
