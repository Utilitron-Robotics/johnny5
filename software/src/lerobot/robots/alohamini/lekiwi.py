...
class AlohaMiniDivergent(Robot):
    """
    ...
    def _smooth_velocity(self, current: float, target: float, alpha: float = 0.2) -> float:
        """Exponential moving average filter for smoothing velocity."""
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        # Apply smoothing
        self.smooth_x = (self.smoothing_alpha * base_goal_vel["x.vel"]) + ((1 - self.smoothing_alpha) * self.smooth_x)
        self.smooth_y = (self.smoothing_alpha * base_goal_vel["y.vel"]) + ((1 - self.smoothing_alpha) * self.smooth_y)
        self.smooth_theta = (self.smoothing_alpha * base_goal_vel["theta.vel"]) + ((1 - self.smoothing_alpha) * self.smooth_theta)

        base_wheel_goal_vel = self._body_to_wheel_raw(
            self.smooth_x, self.smooth_y, self.smooth_theta
        )
    ...
    def read_and_check_currents(self, limit_ma, print_currents):
        """Read left/right bus currents (mA) and voltages, print them, and enforce protection"""
        ...
        # Check voltages for battery safety (Li-ion 3S cutoff ~10.5V)
        # STS3215 voltage unit is 0.1V
        try:
            if self.left_bus.motors:
                first_motor = list(self.left_bus.motors.keys())[0]
                voltage = self.left_bus.read("Present_Voltage", first_motor)
                if voltage < 105: # 10.5V Warning
                    logger.warning(f"LOW BATTERY WARNING: {voltage/10.0}V")
                if voltage < 96: # 9.6V Critical Shutdown
                    logger.error("CRITICAL BATTERY LEVEL - SHUTTING DOWN")
                    self.stop_base()
                    self.disconnect()
        except Exception:
            pass
